...
const DEFAULT_LINE_PREFIX = 'คุณ';
const DEFAULT_LINE_POSITION = 'อื่นๆ';
...
export default function Login() {
  ...
  const syncTeacherProfile = async ({
    ...
    const nextDisplayName = displayName || user?.displayName || profileEmail || 'ผู้ใช้งาน';
    const { firstName, lastName } = splitName(nextDisplayName);

    await ensureTeacherUserProfile({
      user,
      prefix: DEFAULT_LINE_PREFIX,
      firstName,
      lastName,
      position: DEFAULT_LINE_POSITION,
      school: '',
      email: profileEmail,
      photoURL,
      lineUserId,
      ...
      try {
        ...
        if (!user) {
          ...
          try {
            ...
            if (
              ...
              setLineStatus('กำลังสร้างบัญชีผู้ใช้สำหรับ LINE...');
        ...
        if (lineProfile.displayName || lineProfile.pictureUrl) {
          await updateProfile(user, {
            displayName: lineProfile.displayName || user.displayName,
            photoURL: lineProfile.pictureUrl || user.photoURL,
          });
        }
        ...
        await syncTeacherProfile({
          user,
          displayName: lineProfile.displayName || `ผู้ใช้ LINE ${lineProfile.userId}`,
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden font-sans p-4">
      <div className="absolute inset-0 bg-gradient-to-br from-primary/10 via-secondary/10 to-warm/10" />
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-secondary/30 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob" />
      <div className="absolute bottom-[-20%] left-[20%] w-96 h-96 bg-accent/25 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-4000" />

      <div className="relative w-full max-w-sm">
        <div className="absolute inset-0 bg-white shadow-2xl rounded-3xl -rotate-1 opacity-50 transform scale-105" />
          ...
          <div className="text-center mb-6">
            ...
            {!isProcessingLine ? (
              <p className="text-gray-500 text-sm mt-2 font-medium">
                เข้าสู่ระบบการเรียนรู้แห่งอนาคต
              </p>
            ) : null}
          </div>
          ...
          {isProcessingLine ? (
            <div className="flex flex-col items-center justify-center py-8 animate-fade-in">
              <div className="relative mb-6">
                <div className="w-24 h-24 rounded-full border-4 border-gray-100 border-t-[#06C755] animate-spin" />
                <div className="absolute inset-0 flex items-center justify-center">
                  {lineStatus.includes('เสร็จ') ? (
                    <CheckCircle size={40} className="text-[#06C755] animate-bounce" />
                  ) : (
                    <MessageCircle
                      size={36}
                      className="text-[#06C755] animate-pulse"
                      fill="#06C755"
                      color="white"
                    />
                  )}
            ...
            <>
              ...
              <div className="mt-6 text-center">
                <p className="text-xs text-gray-400 font-medium">
                  ...
                  <Link
                    to="/register"
                    className="text-primary font-bold hover:text-blue-700 ml-1 transition-colors"
                  >
                    ลงทะเบียนใหม่
                  </Link>
                </p>
